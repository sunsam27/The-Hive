...
export default function WorkspaceView() {
  ...
  return (
    <AppShell>
                ...
                <div className="data-row-list">
                  ...
                  {expenses.map((e) => (
                    <Link key={e.id} to={`/expenses/${e.id}`} className="data-row-item">
                      ...
                      <span className="data-row-col--amount">{formatCurrency(e.amount, e.currency)}</span>
                      <span>
        ...
        <Modal isOpen={true} onClose={() => setShowSettings(false)} title="Workspace Settings">
          <form onSubmit={async (e) => {
            ...
            {workspace.owner_id === user?.id && (
              <div className="overlay-section--danger">
                <hr className="overlay-divider" />
                <h3 style={{ color: 'var(--color-error)', fontSize: 14, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <AlertTriangle size={16} /> Danger Zone
                </h3>
                <p style={{ fontSize: 13, color: 'var(--color-on-surface-variant)', marginBottom: 12 }}>
                  Deleting this workspace will permanently remove all expenses, receipts, and member associations.
                </p>
                <Button variant="danger" size="sm" onClick={() => { setShowSettings(false); setShowDeleteConfirm(true); }}>
                  <Trash2 size={16} />
                  Delete Workspace
                </Button>
              </div>
      ...
      {showDeleteConfirm && (
        <Modal isOpen={true} onClose={() => { setShowDeleteConfirm(false); setDeleteConfirmName(''); }} title="Delete Workspace">
          <div className="overlay-body">
            <div className="overlay-error" style={{ marginBottom: 16, display: 'flex', alignItems: 'flex-start', gap: 8 }}>
              <AlertTriangle size={18} style={{ flexShrink: 0, marginTop: 2 }} />
              <span>This action cannot be undone. All expenses, receipts, and members will be permanently removed.</span>
            </div>
            <p style={{ fontSize: 14, color: 'var(--color-on-surface-variant)', marginBottom: 12 }}>
            ...
            <input
              ...
              style={{ borderColor: deleteConfirmName === workspace.name ? 'var(--color-error)' : undefined }}
            />
          </div>
          <div className="overlay-footer">
            <Button variant="ghost" onClick={() => { setShowDeleteConfirm(false); setDeleteConfirmName(''); }}>Cancel</Button>
            <Button variant="danger" disabled={deleteConfirmName !== workspace.name || deleting} onClick={async () => {
              setDeleting(true);
              try {
                ...
                showToast(err?.response?.data?.error || 'Failed to delete workspace', 'error');
                setDeleting(false);
              }
            }}>
